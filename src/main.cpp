...
struct ConsoleMode {
    ...
    ConsoleMode() {
        if (out != INVALID_HANDLE_VALUE && GetConsoleMode(out, &oldMode)) {
            DWORD newMode = oldMode | ENABLE_VIRTUAL_TERMINAL_PROCESSING;
            SetConsoleMode(out, newMode);
        }
    }

    ~ConsoleMode() {
        if (out != INVALID_HANDLE_VALUE) {
            SetConsoleMode(out, oldMode);
        }
    }
};
...
struct InputBuffer {
    ...
    void poll() {
        chars.clear();
        while (_kbhit()) {
            chars.push_back(static_cast<char>(_getch()));
        }
    }
};
#else
